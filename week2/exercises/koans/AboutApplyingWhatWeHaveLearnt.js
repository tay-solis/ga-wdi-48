...
describe("About Applying What We Have Learnt", function() {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", function() {
    ...
    productsICanEat = products.filter(function(product) {
      !product.containsNuts && !product.hasMushrooms
    });
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function() {
    ...
    var sum2 = nums.reduce(function(accumulator, currentValue) {
      if (currentValue % 3 === 0 || currentValue % 5 === 0) accumulator += currentValue;
      return accumulator;
  ...
  it("should count the ingredient occurrence (functional)", function() {
    ...
    let ing = products.map(product => product.ingredients).flat();
    let mushCount = ing.reduce(function(accumulator, currentValue){
        if(currentValue === 'mushrooms') accumulator++;
        return accumulator;
    }, 0);
    /* chain() together map(), flatten() and reduce() */

    expect(2).toBe(mushCount);
  });
