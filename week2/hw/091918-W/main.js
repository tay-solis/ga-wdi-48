...
const cat = {
  ...
  makeFriend: function(buddy){
    this.friend = buddy.name;
  }
}
...
class Animal{
  constructor(type, name, noise){
    ...
    this.name = name;
    this.noise = noise;
    this.hunger = Math.floor(10 *  Math.random());
    this.friend = "nobody";
  }
  makeNoise(){
    console.log(this.noise);
...
const riley = new Animal("cat", "Riley", "NyanNyan");
const toast = new Animal("dog", "Toast", "Arf");
const regal = new Animal("cockatiel", "Regal", "Peep");
const muchy = new Animal("mouse", "Munchy", "Chu");
...
*/
for (let i = 0; i < animals.length; i++){
  animals[i].hunger = Math.floor(10 * Math.random());
  console.log(`${animals[i].name} is a ${animals[i].hunger} out of 10 hunger wise.`)
}
...
/*
	...
	// => "chirp quack"
*/
animals[0].makeFriend(animals[1]);
console.log(animals[0].friend);
